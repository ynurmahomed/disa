...
public class ViralLoadFormSchedulerTask extends AbstractTask {
			...
			if (!disaService.existsByRequestId(disa.getRequestId())) {
				...
				if (patientIds.isEmpty()) {
					...
					if (patientIds.size() > 1) {
						...
						// Context.getAdministrationService().getGlobalPropertyObject(Constants.DISA_MAIL_OTHERS_TO).getPropertyValue());
				...
				if (!(disa.getHarvestType() == null)) {
					if (GenericUtil.removeAccents(disa.getHarvestType().trim()).equalsIgnoreCase(Constants.PV)) {// TypeOfSampleCollection
						obs_23824
								.setValueCoded(Context.getConceptService().getConceptByUuid(Constants.VENOUS_PUNCTURE));
						...
						obs_23824.setValueCoded(
								Context.getConceptService().getConceptByUuid(Constants.DIGITAL_PUNCTURE));
						Context.getObsService().saveObs(obs_23824, "");
					}
				}
				...
				if (disa.getFinalViralLoadResult().trim().equalsIgnoreCase(Constants.INDETECTAVEL)) {
					...
					obs_1306.setConcept(
							Context.getConceptService().getConceptByUuid(Constants.HIV_VIRAL_LOAD_QUALITATIVE));
					obs_1306.setValueCoded(
							Context.getConceptService().getConceptByUuid(Constants.UNDETECTABLE_VIRAL_LOAD));
	...
	private void sendEmail(final String message, final String mailTo) {
		 ...
		 * Context.getAdministrationService().getGlobalPropertyObject(Constants.
		...
		GenericUtil.sendMail(
				...
				Context.getAdministrationService().getGlobalPropertyObject(Constants.DISA_MAIL_FROM_PORT)
						.getPropertyValue(),
				Context.getAdministrationService().getGlobalPropertyObject(Constants.DISA_MAIL_FROM_PASSWORD)
						.getPropertyValue());
	}
}